...
import {fitSato} from './sato-fit.mjs';
...
fitSato(scene,skeleton);
for(const b of bones){const n=json.nodes.find(n=>n.name===b.name);n.translation=b.position.toArray();n.rotation=b.quaternion.toArray();n.scale=b.scale.toArray();}
...
for(const side of ['L','R']){
 ...
 feet[side]=bind.get(`${side}_Foot`).point.clone();feet[side].x=side==='L'?.075:-.075;feet[side].y-=sole;feet[side].z=.0194;
}
...
function leg(side,target,pitch=0){
 ...
 const toe=bind.get(`${side}_ToeBase`).point.clone().sub(bind.get(foot.name).point);
 const forwardCorrection=new T.Quaternion().setFromAxisAngle(new T.Vector3(0,1,0),-Math.atan2(toe.x,toe.z));
 setWorld(foot,new T.Quaternion().setFromAxisAngle(new T.Vector3(1,0,0),pitch).multiply(forwardCorrection).multiply(bind.get(foot.name).world));
}
atWorld(bone('Hip'),new T.Vector3(0,.411,.0194));setWorld(bone('Pelvis'),upright);setWorld(bone('Waist'),upright);
for(const side of ['L','R']){
...
function accessor(values,type,min,max,componentType=5126){
 ...
 return json.accessors.push({bufferView,componentType,count:values.length/({SCALAR:1,VEC2:2,VEC3:3,VEC4:4,MAT4:16}[type]),type,...(min?{min,max}:{})})-1;
}
json.skins[0].inverseBindMatrices=accessor(skeleton.boneInverses.flatMap(m=>m.toArray()),'MAT4');
for(const mesh of repair.meshes){
 ...
 if(!geometry.attributes.position.count){for(const n of json.nodes)if(n.mesh===meshIndex){delete n.mesh;delete n.skin;}continue;}
...
const frames=96,stride=.28;json.animations=[];
for(const [name,duration]of [['Idle',4.8],['Walk',.92]]){
 ...
 for(let frame=0;frame<=frames;frame++){
  ...
  hip.y+=walking?-.024-.006*Math.cos(angle*2):.0012*Math.sin(angle);
  ...
  for(const [side,offset]of [['L',0],['R',.5]]){
   ...
   if(walking){target.z=hip.z;if(p<.5)target.z+=stride*(.5-2*p);else{const u=(p-.5)*2;target.z+=stride*(-.5+u-Math.sin(2*Math.PI*u)/Math.PI);target.y+=.068*Math.sin(Math.PI*u)**2;pitch=.12*Math.sin(2*Math.PI*u);}}
...
for(const [side,x]of [['L',.052],['R',-.052]]){
 const pivot=json.nodes.push({name:`EyeSocket_${side}`,translation:new T.Vector3(x,.823,.107).applyMatrix4(headInverse).toArray(),rotation:bind.get('Head').world.clone().invert().toArray(),children:[]})-1;
 (json.nodes[headIndex].children??=[]).push(pivot);
 const socket=json.nodes.push({name:`EyeSkin_${side}`,mesh:eyeMeshes[0],scale:[.019,.019,.002]})-1;
 const blink=json.nodes.push({name:`EyeBlink_${side}`,translation:[0,0,.003],children:[]})-1;
